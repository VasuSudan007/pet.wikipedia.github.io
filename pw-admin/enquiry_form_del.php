...
if($submits=='Activate')
{
$sql="update tbl_enquiry_form set status=1 where id in ($str_rest_refs)";
mysql_query_db($sql);
...
{	
$sql="update tbl_enquiry_form set status=0 where id in ($str_rest_refs)";
mysql_query_db($sql);
...
{
$sql="delete from tbl_enquiry_form where id in ($str_rest_refs)";
mysql_query_db($sql);
...
header("Location: enquiry_form_list.php?start=".$start);
exit();
?>