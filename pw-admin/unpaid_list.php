...
$sql=mysql_query_db("select * from tbl_fees where status=1 order by $order_by $order_by2 limit $start,$pagesize");
$reccnt=mysql_num_db(mysql_query_db("select * from tbl_fees where status=1"));
if($reccnt>0){
$f=1;
...
<?php }else{?>
<span class="label label-sm label-success"><i class="fa fa-times"></i> </span>
<?php }?>
