...
if($id=='')
{
mysql_query_db("insert into tbl_finish set title='".addslashes($title)."',status=1,post_date=now()");
}
else
{
$sql="update tbl_finish set title='".addslashes($title)."'";
$sql.=" where id='$id'";
mysql_query_db($sql);
}
header("location:finish_list.php");
}
if($id)
{
$sql=mysql_query_db("select * from tbl_finish where id='$id'");
