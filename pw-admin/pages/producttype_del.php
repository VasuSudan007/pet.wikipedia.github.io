...
if($submits=='Activate')
{
$sql="update tbl_producttype set status=1 where id in ($str_rest_refs)";
mysql_query_db($sql);
...
{	
$sql="update tbl_producttype set status=0 where id in ($str_rest_refs)";
mysql_query_db($sql);
...
{
$sql="delete from tbl_producttype where id in ($str_rest_refs)";
mysql_query_db($sql);
...
header("Location: producttype_list.php?start=".$start);
exit();
?>