...
if($submits=='Activate')
{
$sql="update tbl_project set status=1 where id in ($str_rest_refs)";
mysql_query_db($sql);
...
{	
$sql="update tbl_project set status=0 where id in ($str_rest_refs)";
mysql_query_db($sql);
...
{
$sql="delete from tbl_project where id in ($str_rest_refs)";
mysql_query_db($sql);
...
header("Location: project_list.php?start=".$start);
exit();
?>