...
$imgae_name=upload_imagen('image',360,233,'../upload_images','../upload_images/thumb');
}
if($id=='')
{
mysql_query_db("insert into tbl_banner set title='$title',image='$imgae_name',status=1,post_date=now(),page_name='$page_name'");
}
else
{
$sql="update tbl_banner set title='$title',page_name='$page_name'";
if($imgae_name)
{
...
if($id)
{
$sql=mysql_query_db("select * from tbl_banner where id='$id'");
...
<div class="form-group">
<label class="col-sm-3 control-label no-padding-right" for="form-field-1"> Image <span class="red">*</span></label>
<div class="col-sm-9">
<input type="file" name="image" id="image" style="width:350px; height:30px; padding:5px; border:1px solid #cccccc;" title="Please enter image">
<?php if($line['image']!='')
{?>
<br>
</br>
...
<div class="form-group">
<label class="col-sm-3 control-label no-padding-right" for="form-field-1-1">Link<span class="red">*</span></label>
<div class="col-sm-9">
<input type="text" id="page_name" value="<?php echo $page_name;?>" name="page_name" title="please enter link" placeholder="Enter Link" class="col-xs-10 col-sm-4" /> 
</div>
</div>
