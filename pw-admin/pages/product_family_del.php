...
if($submits=='Activate')
{
$sql="update tbl_product_family set status=1 where id in ($str_rest_refs)";
mysql_query_db($sql);
...
{	
$sql="update tbl_product_family set status=0 where id in ($str_rest_refs)";
mysql_query_db($sql);
...
{
$sql="delete from tbl_product_family where id in ($str_rest_refs)";
mysql_query_db($sql);
...
header("Location: product_family_list.php?start=".$start);
exit();
?>