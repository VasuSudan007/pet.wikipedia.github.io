...
if($id=='')
{
mysql_query_db("insert into tbl_cri set title='".addslashes($title)."',status=1,post_date=now()");
}
else
{
$sql="update tbl_cri set title='".addslashes($title)."'";
$sql.=" where id='$id'";
mysql_query_db($sql);
}
header("location:cri_list.php");
}
if($id)
{
$sql=mysql_query_db("select * from tbl_cri where id='$id'");
