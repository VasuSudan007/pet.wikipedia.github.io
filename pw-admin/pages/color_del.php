...
if($submits=='Activate')
{
$sql="update tbl_color_temp set status=1 where id in ($str_rest_refs)";
mysql_query_db($sql);
...
{	
$sql="update tbl_color_temp set status=0 where id in ($str_rest_refs)";
mysql_query_db($sql);
...
{
$sql="delete from tbl_color_temp where id in ($str_rest_refs)";
mysql_query_db($sql);
...
header("Location: color_list.php?start=".$start);
exit();
?>