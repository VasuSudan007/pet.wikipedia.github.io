...
if($id=='')
{
mysql_query_db("insert into tbl_ip_rating set title='".addslashes($title)."',status=1,post_date=now()");
}
else
{
$sql="update tbl_ip_rating set title='".addslashes($title)."'";
$sql.=" where id='$id'";
mysql_query_db($sql);
}
...
if($id)
{
$sql=mysql_query_db("select * from tbl_ip_rating where id='$id'");
...
<li class="active">Add IP Rating</li>
</ul><!-- /.breadcrumb -->
</div>
