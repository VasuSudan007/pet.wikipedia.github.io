...
if($submits=='Activate')
{
$sql="update tbl_beam_angle set status=1 where id in ($str_rest_refs)";
mysql_query_db($sql);
...
{	
$sql="update tbl_beam_angle set status=0 where id in ($str_rest_refs)";
mysql_query_db($sql);
...
{
$sql="delete from tbl_beam_angle where id in ($str_rest_refs)";
mysql_query_db($sql);
...
header("Location: beam_list.php?start=".$start);
exit();
?>