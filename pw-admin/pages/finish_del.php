...
if($submits=='Activate')
{
$sql="update tbl_finish set status=1 where id in ($str_rest_refs)";
mysql_query_db($sql);
...
{	
$sql="update tbl_finish set status=0 where id in ($str_rest_refs)";
mysql_query_db($sql);
...
{
$sql="delete from tbl_finish where id in ($str_rest_refs)";
mysql_query_db($sql);
...
header("Location: finish_list.php?start=".$start);
exit();
?>