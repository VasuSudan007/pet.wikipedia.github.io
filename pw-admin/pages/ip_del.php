...
if($submits=='Activate')
{
$sql="update tbl_ip_rating set status=1 where id in ($str_rest_refs)";
mysql_query_db($sql);
...
{	
$sql="update tbl_ip_rating set status=0 where id in ($str_rest_refs)";
mysql_query_db($sql);
...
{
$sql="delete from tbl_ip_rating where id in ($str_rest_refs)";
mysql_query_db($sql);
...
header("Location: ip_list.php?start=".$start);
exit();
?>