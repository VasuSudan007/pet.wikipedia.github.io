...
if($id=='')
{
mysql_query_db("insert into tbl_brand set title='".addslashes($title)."',status=1,content='".addslashes($content)."'post_date=now()");
}
else
{
$sql="update tbl_brand set title='".addslashes($title)."'";
$sql.=" where id='$id'";
mysql_query_db($sql);
}
...
if($id)
{
$sql=mysql_query_db("select * from tbl_brand where id='$id'");
...
<div class="col-sm-9">
<textarea name="content" class="form-control"><?php echo stripslashes($content);?></textarea> 
</div>
</div>
