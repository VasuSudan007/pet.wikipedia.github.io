...
if($submits=='Activate')
{
$sql="update tbl_fees set status=1 where id in ($str_rest_refs)";
mysql_query_db($sql);
...
{	
$sql="update tbl_fees set status=0 where id in ($str_rest_refs)";
mysql_query_db($sql);
...
{
$sql="delete from tbl_fees where id in ($str_rest_refs)";
mysql_query_db($sql);
