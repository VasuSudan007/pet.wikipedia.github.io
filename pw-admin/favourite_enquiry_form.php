...
frmobj.action = "enquiry_form_del.php";
frmobj.submit();
}
else{ 
return false;
...
location.href='enquiry_form_del.php?submits=Delete&ids[]='+vla;
}
</script>
...
<th class="center">Fathers Name</th>
...
$sql=mysql_query_db("select * from tbl_enquiry_form where status=0 $wh order by $order_by $order_by2 limit $start,$pagesize");
$reccnt=mysql_num_db(mysql_query_db("select * from tbl_enquiry_form where 1=1 $wh"));
if($reccnt>0){
$f=1;
...
<td class="center"><?php echo ucwords(stripslashes($line['sname']));?></td>
<td class="center"><?php echo ucwords(stripslashes($line['class']));?></td>
<td class="center"><?php echo ucwords(stripslashes($line['fname']));?></td>
<td class="center"><?php echo ucwords(stripslashes($line['phone']));?></td>
<td class="center"><?php echo ucwords(stripslashes($line['email']));?></td>
<td class="center"><?php echo ucwords(stripslashes($line['post_date']));?></td>

<td class="center">
<?php if($line['status']==1){?>
<span class="label label-sm label-success"></span>
<?php }else{?>
<span class="label label-sm label-warning"><i class="fa fa-star"></i> </span>
<?php }?>
...
<button class="btn btn-xs btn-info" type="button" onClick="location.href='enquiry_form_addf.php?id=<?php echo $line['id'];?>&start=<?php echo $start;?>'">
<i class="ace-icon fa fa-eye bigger-120"> Show</i>
</button>

<button class="btn btn-xs btn-danger" name="del" value="Delete" type="button" onclick="checkauto('<?php echo $f;?>','<?php echo $line['id']?>')">
<i class="ace-icon fas fa-trash bigger-120"></i>
</button>
...
<span class="blue">
<i class="ace-icon fa fa-eye-square-o bigger-120">View</i>
</span>
</a>
</li>
...
<button class="btn btn-success" name="submits" onclick="return del_prompt(this.form,this.value,'favourite_enquiry_form.php')" type="submit" value="Activate">Mark as unfavourite</button>
<button class="btn btn-warning" name="submits" onclick="return del_prompt(this.form,this.value,'favourite_enquiry_form.php')" type="submit" value="Deactivate">Mark as favourite </button>
<button class="btn btn-danger" name="submits" onclick="return del_prompt(this.form,this.value,'enquiry_form_list.php')" type="submit" value="Delete">Delete</button>
</td></tr>
<?php }else{?>
